...
def get_port_number() -> int:
    ...
    """
    return flslDevice.getPortNumber()
...
def midi_out_msg(message: int) -> None:
    ...
    - None
    """
    return flslDevice.midiOutMsg(message)
...
def midi_out_msg_params(midi_id: int, channel: int, data1: int, data2: int) -> None:
    ...
    - None
    """
    return flslDevice.midiOutMsg(midi_id, channel, data1, data2)
...
def midi_out_new_msg(slot_index: int, message: int) -> None:
    ...
    - None
    """
    return flslDevice.midiOutNewMsg(slot_index, message)
...
def midi_out_sysex(message: str) -> None:
    ...
    - None
    """
    return flslDevice.midiOutSysex(message)
...
def send_msg_generic(id: int, message: str, last_msg: str, offset: int = 0) -> str:
    ...
    """
    return flslDevice.sendMsgGeneric(id, message, last_msg, offset)
...
def find_event_id(control_id: int, flags: int = []) -> int:
    ...
    """
    return flslDevice.findEventID(control_id, flags)
...
def get_linked_value(event_id: int) -> float:
    ...
    """
    return flslDevice.getLinkedValue(event_id)
...
def get_linked_value_string(event_id: int) -> str:
    ...
    - str: Text value.
    """
    return flslDevice.getLinkedValueString(event_id)
...
def get_linked_channel(event_id: int) -> int:
    ...
    """
    return flslDevice.getLinkedChannel(event_id)
...
def get_linked_param_name(event_id: int) -> str:
    ...
    - str: Parameter name.
    """
    return flslDevice.getLinkedParamName(event_id)
...
def get_linked_info(event_id: int) -> int:
    ...
    """
    return flslDevice.getLinkedInfo(event_id)
...
def link_to_last_tweaked(control_index: int, channel: int, global_: int, event_id: int) -> int:
 ...
 or (2) when the control is in use.
    ...
    """
    return flslDevice.linkToLastTweaked(control_index, channel, global_, event_id)
...
def get_device_id() -> bytes:
    ...
    - bytes: Device ID.
    """
    return flslDevice.getDeviceID()

# Refresh thread

def create_refresh_thread() -> None:
    """
    Start a threaded refresh of the entire MIDI flslDevice.

    Returns:
    - None
    """
    return flslDevice.createRefreshThread()
...
def destroy_refresh_thread() -> None:
    ...
    - None
    """
    return flslDevice.destroyRefreshThread()
...
def is_double_click(index: int) -> int:
    ...
    """
    return flslDevice.isDoubleClick(index)
...
def set_has_meters() -> None:
    ...
    - None
    """
    return flslDevice.setHasMeters()
...
def base_track_select(index: int, step: int) -> None:
    ...
    - None
    """
    return flslDevice.baseTrackSelect(index, step)
...
def hardware_refresh_mixer_track(index: int) -> None:
    ...
    - None
    """
    return flslDevice.hardwareRefreshMixerTrack(index)
...
def dispatch(ctrl_index: int, message: int, sysex: bytes) -> None:
    ...
    - None
    """
    return flslDevice.dispatch(ctrl_index, message, sysex)
...
def dispatch_receiver_count() -> int:
    ...
    """
    return flslDevice.dispatchReceiverCount()
...
def dispatch_get_receiver_port_number(ctrl_index: int) -> int:
    ...
    """
    return flslDevice.dispatchGetReceiverPortNumber(ctrl_index)

def set_master_sync(value: int) -> None:
    """
    Toggle (value = 1 to enable, 0 to disable) send master synch for the current flslDevice.
    ...
    - None
    """
    return flslDevice.setMasterSync(value)
...
def get_master_sync() -> int:
    ...
    """
    return flslDevice.getMasterSync()